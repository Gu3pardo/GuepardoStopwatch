...
public class SharedPrefController {
    ...
    public SharedPrefController(
            @NonNull Context context,
            @NonNull String preferenceName) {
        _logger = new Logger(TAG);
        _logger.Debug("Created new " + TAG + "...");

        _sharedPreferences = context.getSharedPreferences(preferenceName, 0);
        _logger.Debug("PreferenceName is " + preferenceName);
    }
    ...
    public boolean Contains(@NonNull String key) {
        ...
        _logger.Warn("Key not available!");
        return false;
    }
    ...
    public boolean SaveStringValue(
        ...
        if (_sharedPreferences == null) {
            _logger.Warn("SharedPreferences is null!");
            return false;
        }
